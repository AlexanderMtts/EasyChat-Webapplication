...
app.get('/', function (req, res, next) {
    res.json({ message: 'EasyChat API works successfully' });
});
...
app.post('/history', function (req, res, next) {
    ...
    chatHistory.k({ message: req.body.message, nickname: req.body.nickname, date: date });
...
app.listen(app.get('port'), function () {
    console.log('Node app is running successfully on port', app.get('port'));
});
